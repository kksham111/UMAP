...
def eval_one_epoch(config, data_loader, device, model, action_normalizer=None):
    ...
    for batch in tqdm(data_loader, desc="Evaluating", disable=not is_main_process()):
        ...
        with torch.no_grad():
            ...
            next_img = model.obs_encoder.apply_transform(next_obs_dict)
            next_latent = model.obs_encoder.apply_vae(next_img)

            # Sample next obs and actions from joint distribution
            next_latent_hat_joint, action_hat_joint = model.sample_joint(curr_obs_dict)
            joint_image_mse = F.mse_loss(next_latent_hat_joint, next_latent)
    ...
    # Plot reconstruction
    stats["images"] = wandb.Image(decode_and_plot(next_latent[0:1]))
    stats["images_joint"] = wandb.Image(decode_and_plot(next_latent_hat_joint[0:1]))
    return stats
...
@hydra.main(
    version_base=None, config_path="../../configs", config_name="train_pad.yaml"
)
def main(config):
